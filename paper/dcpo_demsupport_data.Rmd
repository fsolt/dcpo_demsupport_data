...
# define functions
validation_plot <- function(v_data_raw, v_var, v_val, neg = FALSE,
                            lab_x, lab_y, show_n = FALSE,
                            theta_summary, theta_results) {
    
    if (missing(lab_x)) lab_x <- .7
    if (missing(lab_y)) lab_y <- .97
...
validation_asianb_raw <- "data-raw/validation_items_asianb.csv" %>%
    ...
    DCPOtools::dcpo_setup(datapath = here("..",
                                          ...
                                          "dcpo_surveys"),
                          file = here("data",
                                      "dcpo_input_asianb.csv")) %>% 
  filter(!country == "Hong Kong SAR China") %>% 
  left_join(read_csv("data-raw/validation_asianb_titles.csv",
                     col_types = "cc"), by = "item")

econ_priority_plot0 <- validation_plot(validation_asianb_raw,
                                      "econ_priority4a", 3) 
econ_priority_plot <- econ_priority_plot0 +
    theme_bw() +
    theme(legend.position="none",
    ...
    labs(x = "Democratic Support Score",
         y = '"If You Had to Choose Between Democracy and Economic Development,\nWhich Would You Say Is More Important?"\n% Choosing Democracy') +
    ggtitle("Asian Barometer, Five Waves")

leg_check_plot0 <- validation_plot(validation_asianb_raw,
                                      "legislative_check4", 3) 

leg_check_plot <- leg_check_plot0 +
  theme_bw() +
  theme(legend.position="none",
        axis.text  = element_text(size=8),
        axis.title = element_text(size=9),
  ...
  coord_cartesian(ylim=c(0,100)) +
  labs(x = "Democratic Support Score",
       y = '% Disagreeing that "If the Government Is Constantly Checked by the\nLegislature, It Cannot Possibly Accomplish Great Things."') +
  ggtitle("Asian Barometer, Five Waves")
...
asianb_v_vars <- c("dem_capable2", 
                   {validation_asianb_raw %>% 
                       filter(str_detect(item, "4")) %>% 
                       pull(item) %>% 
                       unique() %>% 
                       `[`(1:14)})

all_plots <- purrr::map2(asianb_v_vars,
                        c(2, rep(3, 14)),
                        ~validation_plot(validation_asianb_raw,
                                .x, .y, neg = TRUE) +
      theme_bw() +
      theme(legend.position="none",
            axis.text  = element_text(size=8),
...
do.call(gridExtra::grid.arrange, c(all_plots, ncol = 5))

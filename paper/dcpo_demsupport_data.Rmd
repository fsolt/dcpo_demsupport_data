...
title: |
  | Unidimensional Latent Variable Measures of
...
-   Hyein Ko, ORCID: <https://orcid.org/0000-0002-9497-9656>, Postdoctoral Fellow, Department of Political Science, University of Illinois, [hyeink\@illinois.edu](mailto:hyeink@illinois.edu){.email}
...
As will be shown later, most surveys gather data on democratic support using Churchill-style questions asking public attitude in the abstract without considering concrete components in democracy, which force respondents to place themselves on a scale between complete support and full opposition. 
When the meaning of democracy is not consistent, respondents' answers may be based on different cognition or experiences. 

To some extent, the novel latent-variable methods are designed to address these issues. 
As long as the questions, regardless of their format, *aim to measure the same underlying latent dimension*, these methods employ statistical tools to map diverse responses onto a common scale and estimate the probability that a respondent with a given level of the latent variable would answer different questions in a particular way. 
The differences in question format, style, and other aspects are accounted for with parameters such as difficulty and discrimination. 
...
Finally, a third possible reason why the new methods do not perform as expected is when their underlying assumptions are violated. 

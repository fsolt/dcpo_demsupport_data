...
```{r polyarchy, cache = TRUE, fig.cap="Construct Validation: Correlations Between Democratic Support as a Unidimensional Latent Variable and Polyarchy Survey Items \\label{polyarchy}", fig.height=6}
v_titles <- "data-raw/validation_items_fs.csv" %>%
    read_csv(col_types = "ccccccc") %>%
    select(item, title, neg, group, svy_grp) %>%
    distinct() %>% 
    mutate(title = paste0(svy_grp, ": ", title))

v_raw <- "data-raw/validation_items_fs.csv" %>%
    read_csv(col_types = "ccccccc") %>%

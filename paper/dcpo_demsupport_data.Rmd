...
fontsize: 11pt
spacing: double
bibliography: dcpodemsupportdata.bib
...
```{r v_data, include=FALSE, cache=TRUE}
v_titles <- here("data-raw", "validation_items_fs.csv") %>%
    read_csv(col_types = "ccccccc") %>%
    select(item, title, neg, group, svy_grp) %>%
    distinct() %>% 
    mutate(title = paste0(svy_grp, ": ", title))

v_raw <- here("data-raw", "validation_items_fs.csv") %>%
    read_csv(col_types = "ccccccc") %>%
...
```{r polyarchy, fig.cap="Construct Validation: Correlations Between Democratic Support as a Unidimensional Latent Variable and Polyarchy Survey Items \\label{polyarchy}", fig.height=3, cache=TRUE}
v_vars_poly <- v_raw %>%
    filter(group == "contest" | group == "participation") %>% 
    arrange(group, title) 

validation_plot(v_vars_poly, lab_x = .5) +
  theme_bw() +
  theme(legend.position="none",
...
```{r liberties, cache = TRUE, fig.cap="Construct Validation: Correlations Between Democratic Support as a Unidimensional Latent Variable and Civil Liberties Survey Items \\label{polyarchy}", fig.height=4.5}
...
```{r institutions, cache = TRUE, fig.cap="Construct Validation: Correlations Between Democratic Support as a Unidimensional Latent Variable and Democratic Institutions Survey Items \\label{polyarchy}", fig.height=3}

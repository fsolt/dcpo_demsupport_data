...
It is important to acknowledge that the latent variable estimates are based on the assumption that democratic support falls along a single dimension: some individuals and some population support democracy wholeheartedly, others fully oppose democracy, and everyone else is somewhere in between on this spectrum. 
This underlying assumption raises an important question about how we conceptualize and measure democracy. 
In the studies of democracies and various measures of democracy, the initial step is how we understand democracy itself. 
Varieties of Democracy (V-Dem), for instance, anchors their theoretical ground to @Dahl1971 and conceptualizes democracy in a multifacated way by distinguishing between five principles – electoral, liberal, participatory, deliberative, and egalitarian [@VDem2023data; @VDem2023codebook]. 
Based on the expert surveys, V-Dem provides measures along with these dimensions. 
In other two commonly used measures of democracy, namely Freedom House and Polity Score, are focusing on different aspects of democracy. While the previous examples are primarily about conceptualization and datasets, it is widely know that there are competing models of democracy. 
Likewise, research in comparative public opinion has been cautious about fluidity in the public's perception of democracy. 
@Bratton2002, for example, illustrated that in some countries, the public will attach "homeground meaning to democracy" (p.5). 
Much research on the democratic support further points out that we cannot merely assume that survey questions regarding democratic support will mean the same thing to everyone [e.g., @Schedler2007; @Konig2022]; taken together, previous studies argues that support on democracy is a more complex and multidimensional phenomenon. 

...
-   Yue Hu, corresponding author, ORCID: <https://orcid.org/0000-0002-2829-3971>, Associate Professor, Department of Political Science, Tsinghua University, [yuehu\@tsinghua.edu.cn](mailto:yuehu@tsinghua.edu.cn){.email}
...
Recent threats to liberal democracy have manifested as the slow erosion of institutions and norms rather than the sudden and violent takeover of power, as the acts of politicians and elected officials rather than admirals and generals.
In this context, considering that the ostensible backing for these antidemocratic office holders is some substantial share of the electorate, understanding the breadth and depth of citizen support for democracy is as important as ever.
...
Here, we test the validity of the sort of unidimensional measure used in recent latent-variable-driven research by comparing it to responses to questions drawn from many surveys used in the broader literature to map the dimensions of support for liberal democracy and opposition to its erosion.
...
More generally, this finding highlights how researchers creating latent variables must be attentive to issues of multidimensionality and provide ample validation for their measures.
...
# The Unidimensional Latent Variable Approach to Measuring Democratic Support {.unnumbered}

To provide the strongest test of the validity of the unidimensional latent variable of democratic support, we must create the best version of such a variable currently possible.
...
Finally, we estimated a unidimensional latent variable of democratic support from these responses using the DCPO model presented in @Solt2020c; that work shows that this model provides a better fit to survey questions on express support for democracy in the abstract than the model employed in the existing research on this topic, Claassen's [-@Claassen2019] Model 5.
...
To test the validity of the unidimensional latent variable measure of democratic support, we compare it to a range of survey questions that also address various aspects of support for democracy; that is, we provide a series of tests of convergent validation [see @Adcock2001, 540].
Because, following the existing works employing such a measure, we did not include these survey questions as indicators when we estimated the latent variable, these constitute "external" validation tests [see @Caughey2019, 684-685].
Where the indicators used to estimate the latent variable expressly ask respondents to evaluate "democracy" or a specific undemocratic alternative, however, the questions we use for these tests probe further.
"fully contested elections with full suffrage and the absence of massive fraud, combined with effective guarantees of civil liberties, including freedom of speech, assembly, and association" [@Collier1997, 434]. **<- Put this all directly in reference to Dahl**

We start with survey questions that most directly address Dahl's [-@Dahl1971, 4] two "theoretical dimensions of democratization," public constestation and inclusive participation.
For example, the AmericasBarometer asks whether chief executives should "limit the voice and vote of opposition parties," and the AsianBarometer questions whether "people with little or no education should have as much say in politics as highly-educated people," and at the two dimensions' point of greatest overlap, the Afrobarometer asks whether "we should choose our leaders in this country through regular, open and honest elections."
Scatterplots of the aggregate responses to these questions and other similar items against the unidimensional latent variable of support for democracy in the abstract are presented in Figure\nobreakspace{}\@ref(fig:polyarchy).
...
```{r polyarchy, fig.cap="Correlations Between Democratic Support as a Unidimensional Latent Variable and Polyarchy Survey Items \\label{polyarchy}", fig.height=3, cache=FALSE}
v_vars_poly <- v_raw %>%
...
```{r liberties, cache = FALSE, fig.cap="Correlations Between Democratic Support as a Unidimensional Latent Variable and Civil Liberties Survey Items \\label{liberties}", fig.height=4.5}
...
Again drawing on multi-wave cross-national surveys, we compiled four items regarding free speech, three on freedom of the press, three more on freedom of assembly, and two that deal with freedom of association.
Scatterplots of the aggregate responses to these questions against the unidimensional latent variable of democratic support are presented in Figure\nobreakspace{}\@ref(fig:liberties).
All of these correlations are weak to very weak, and several are in the opposite of the expected direction.
For example, across five waves of the Asian Barometer (2003-2018), the unidimensional latent variable of democratic support has a _positive_ relationship ($R~=~0.28$) with aggregate agreement with the statement, "The government should decide whether certain ideas should be allowed to be discussed in society."
...
```{r institutions, cache = FALSE, fig.cap="Correlations Between Democratic Support as a Unidimensional Latent Variable and Democratic Institutions Survey Items \\label{institutions}", fig.height=3}
...
Figure\nobreakspace{}\@ref(fig:institutions)

```{r conditions, cache = FALSE, fig.cap="Correlations Between Democratic Support as a Unidimensional Latent Variable and Conditionality Survey Items \\label{conditions}", fig.height=3}
...
As mentioned above, recent research has documented that many who proclaim support for democracy in the abstract nevertheless have other preferences---such as those regarding policy or partisanship---that they may consider to be more important priorities [see @Graham2020; @Simonovits2022; however, for the argument that people rarely consciously trade off democracy for other priorities but instead rationalize that their values do not conflict, see @Krishnarajan2022].
...
Advances in computational hardware and software have made estimating latent variables a powerful tool for studying public opinion.
...
As is the case for any newly proposed measure, validation tests of latent variables are crucially important.
examples: @Caughey2019; @Woo2022.
...
We created the unidimensional latent variable of democratic support used here by first collecting survey questions that asked respondents to choose betweeen democracy and an undemocratic alternative or to evaluate either democracy or one of these undemocratic alternatives. 
...
These data are sparse, but they do include `r n_cy` country-years for which we have at least *some* information about the extent of support for democracy, that is, some `r round(covered_share_of_spanned)`% of the `r spanned_cy` country-years spanned by the data we collected.
...
We estimated the model using the `DCPOtools` and `DCPO` packages for R [@Solt2019; @Solt2020a], running four chains for 1,000 iterations each and discarding the first half as warmup, which left us with 2,000 samples.
...
Figure\nobreakspace{}\ref{cs_mry} displays the most recent available DSD score for each of the `r res_c` countries and territories in the dataset.

The latest scores for `r bottom5` have them as the places where the public expresses the lowest support for democracy.

```{r ts, fig.cap="The Unidimensional Latent Variable of Democratic Support Over Time Within Selected Countries \\label{ts}", fig.height=3.5}
...
To provide the best test of the unidimensional latent variable approach for measuring democratic support, in main text the latent variable is estimated using the largest indicator dataset currently available and the DCPO model, which @Solt2020c demonstrates fits these data better than the alternatives.
However, the existing scholarship taking this approach uses a considerably smaller dataset of responses to these indicators and Claassen's [-@Claassen2019] Model 5 [see @Claassen2020a; @Claassen2020b; @Tai2022; @Claassen2022].
...
```{r m5-polyarchy, fig.cap="Correlations Between Democratic Support as a Unidimensional Latent Variable and Polyarchy Survey Items", fig.height=3, cache=FALSE}
...
```{r m5-liberties, cache = FALSE, fig.cap="Correlations Between Democratic Support as a Unidimensional Latent Variable and Civil Liberties Survey Items", fig.height=4.5}
...
Here, the only moderate and strong correlations are actually in the oppposite of the expected direction: greater support for democracy is associated with a stronger approval to ban protests and to use the military to repress strikes.
...
```{r m5-institutions, cache = FALSE, fig.cap="Correlations Between Democratic Support as a Unidimensional Latent Variable and Democratic Institutions Survey Items", fig.height=3}
validation_plot(v_vars_inst,
                lab_x = -1,
                theta_summary = m5_theta_summary,
                theta_results = m5_theta_results) +
...
Support for the rule of law relates appreciably to this measure of democratic support in only the AfroBarometer; in the AsianBarometer, Latinobarométro, and Pew Global Attitudes surveys, the correlations are very weak.
Democratic support so measured is similarly weakly related to data from questions tapping support for the legislative branch in the AfroBarometer, AmericasBarometer, and AsianBarometer, with only the Latinobarométro question on whether national legislatures are necessary at all for democracy showing even a moderate relationship.

```{r m5-conditions, cache = FALSE, fig.cap="Correlations Between Democratic Support as a Unidimensional Latent Variable and Conditionality Survey Items \\label{m5conditions}", fig.height=3}
...
As when estimated with more data and the superior DCPO model, the more limited unidimensional latent variable democratic support has only very weak relationships with the survey questions that ask respondents to choose between order, economic equality, or economic growth, on the one hand, and democracy or political freedom, on the other.
